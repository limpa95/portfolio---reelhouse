...
// Fetch requests have been customized in order to create dropdown lists for Users, Movies, and TV Shows. Toggling a view will update the total views column in either the movies or televisions table.
...
function CreateEngagement() {
  ...
  const handleSubmit = async (e) => {
    ...
    if(newEngagement.view === 1 && newEngagement.television_id !== ""){
    try {
      const URL = import.meta.env.VITE_API_URL + "engagements/" + "televisionviews/" + newEngagement.television_id;
      const response = await axios.put(URL, newEngagement);
    ...
    if(newEngagement.view === 1 && newEngagement.movie_id !== ""){
      try {
        const URL = import.meta.env.VITE_API_URL + "engagements/" + "movieviews/" + newEngagement.movie_id;
        const response = await axios.put(URL, newEngagement);
        if (response.status !== 200) {
  ...
  const handleDropdownChangeMovie = (e) => {
    ...
    let total_view = "";
    Movies.map((val, i) => {
                if (val.movie_title === value){id = val.movie_id; total_view = val.movie_total_view}
      }
    )
  ...
  const handleDropdownChangeTVShow = (e) => {
    ...
    let total_view = "";
    TVShows.map((val, i) => {
                if (val.television_title === value){id = val.television_id; total_view = val.television_total_view}
      }
    );
    total_view = total_view + 1;
    ...
    setFormData((prevData) => ({
      ...prevData,
      [name]: id,
      television_total_view: total_view
    }));
  };
